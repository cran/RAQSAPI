...
These are all the available variables that can be used with functions exported
from the RAQSAPI library listed alphabetically.
* AQSobject: a object that is an AQS_DATAMART_APIv2 S3 object.
...
* countycode: a R character object which represents the 3 digit state FIPS code
                ...
                [aqs_counties_by_state()] for the list of available county
...
str_subset(string = RAQSAPI_functions, pattern = listfunctions) %>%
  cat(sep = "  \n")
```
List functions return options or groupings that can be used in conjunction with
    other API calls. By default each function in this category returns results
    as a tibble. If return_header parameter is set to TRUE a AQSAPI_v2 object is
    returned instead.
...
RAQSAPI::aqs_classes takes no arguments and returns a table of parameter
    classes (groups of parameters, i.e. "criteria" or "all"). The information
    from this function can be used as input to other API calls.
...
3. **Daily Summary Data**: Returns data summarized at the daily level. All daily
                            ...
                            using this service contain *\*dailydata_by_*\* in
...
4. **Annual Summary Data**: Returns data summarized at the yearly level.
      ...
      + cedate: a R date object which represents an "end date of last change"
                  ...
                  changed on or before this date will be returned. This is an
                  optional variable which defaults to NA_Date_.
5. **Quality Assurance - Blanks Data**:
...
9. **Quality Assurance - One Point Quality Control Raw Data**:
                      ...
                      name.  
10. **Quality Assurance - pep Audits**:
...
* cbdate (optional): (This parameter is only used in conjunction with 
                        *\*sampledata_by_*\*, *\*dailydata_by_*\*,
                        *\*annualdata_by_*\* functions).
...
* MA_code:
* cbdate (optional): (This parameter is only used in conjunction with 
                        *\*sampledataby*\*, *\*dailydataby*\*,
                        *\*annualdataby*\* functions).
* cedate (optional): (This parameter is only used in conjunction with
                        *\*sampledataby*\*, *\*dailydataby*\*,
                        *\*annualdataby*\* functions).
* return_header (optional): set to FALSE by default.
...
functions in this family of functions aggregate data at the Core Based
  Statistical Area (cbsa) level. All functions accept the following variables:
...
This is useful if the user has saves the output from another RAQSAPI function
