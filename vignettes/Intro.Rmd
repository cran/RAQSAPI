...
The RAQSAPI package for the R programming environment allows a R programming
  ...
  API v2 ^[[Air Quality System (AQS) API](
  https://aqs.epa.gov/aqsweb/documents/data_api.html)] interface directly. This
  package enables the data user to omit legacy challenges including coercing
  data from a JSON object to a usable R object, retrieving multiple years of
  data, formatting API requests, retrieving results, handling credentials,
  requesting multiple pollutant data and rate limiting data requests. All the
  basic functionality of the API have been implemented that are available from
  the AQS API Data Mart server. The library connects to AQS Data Mart API via
  Hypertext Transfer Protocol (HTTP) so there is no need to install external
  ...
  return_header which by default is set to FALSE. If the user
  ...
  tibble ^[see (https://tibble.tidyverse.org) for more information about
  tibbles.] which contains the header information. The Header contains status
